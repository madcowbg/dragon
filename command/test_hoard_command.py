...
class TestHoardCommand(unittest.TestCase):
    ...
    def test_sync_to_hoard(self):
        ...
        hoard = hoard_cmd.hoard
        with HoardContents.load(hoard.hoardpath) as hoard_contents:
    ...
    def test_sync_two_repos(self):
        ...
        cave_cmd2.init()
        cave_cmd2.refresh(show_details=False)
        ...
        hoard = hoard_cmd.hoard
        with HoardContents.load(hoard.hoardpath) as hc:
            self._assert_hoard_contents(
                hc,
                files_exp=[
                    ('/wat/test.me.different', 5, 1, '5a818396160e4189911989d69d857bd2'),
        ...
        hoard2 = hoard_cmd.hoard
        with HoardContents.load(hoard2.hoardpath) as hc:
            self._assert_hoard_contents(
                hc,
                files_exp=[
    ...
    def test_populate_one_repo_from_other_repo(self):
        ...
        orig_cave_cmd.init()
        orig_cave_cmd.refresh(show_details=False)
...
def init_complex_hoard(tmpdir: str):
    ...
    backup_cave_cmd.refresh(show_details=False)
