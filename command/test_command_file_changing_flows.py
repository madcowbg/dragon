...
class TestFileChangingFlows(IsolatedAsyncioTestCase):
    ...
    async def test_initial_population(self):
        ...
        self.assertEqual((
            ...
            'Remote repo-full-name current=b2effc staging=b2effc desired=7672cb\n'
            'Remote repo-incoming-name current=None staging=None desired=None\n'
            'Remote repo-partial-name current=499273 staging=499273 desired=238de4\n'
            'repo-partial-name:\n'
            'REMOTE_COPY [repo-full-name] wat/test.me.z\n'
            'repo-partial-name:\n'
            'After:\n'
            'Remote repo-backup-name current=None staging=None desired=7672cb\n'
            'Remote repo-full-name current=b2effc staging=b2effc desired=7672cb\n'
            'Remote repo-incoming-name current=None staging=None desired=None\n'
            'Remote repo-partial-name current=238de4 staging=499273 desired=238de4\n'
            'DONE'), res)
        ...
        res = await hoard_cmd.contents.drop("repo-partial-name", "wat")
        self.assertEqual((
            'DROP /wat/test.me.2\n'
            'DROP /wat/test.me.z\n'
        ...
        self.assertEqual((
            ...
            'Hoard root: 7672cb5914a2bb1ae0ba43506eeb54f6fbeb5ad9:\n'
            'Repo current=238de4 staging=499273 desired=4ca939\n'
            'Repo root: 499273be7613adfbfa809a42a7d2b6e9be0245e6:\n'
            'REPO_FILE_TO_DELETE /wat/test.me.2\n'
            'REPO_FILE_TO_DELETE /wat/test.me.z\n'
            'HOARD_FILE_DELETED /wat/test.me.z\n'), res)

        res = await hoard_cmd.files.pending("repo-partial-name")
        self.assertEqual((
            'repo-partial-name:\n'
            'TO_RETAIN (needed in 1 [repo-backup-name]) /wat/test.me.2\n'
            'TO_RETAIN (needed in 1 [repo-backup-name]) /wat/test.me.z\n'
            'DONE'), res)
        ...
        self.assertEqual((
            ...
            'Remote repo-full-name current=b2effc staging=b2effc desired=7672cb\n'
            'Remote repo-incoming-name current=None staging=None desired=None\n'
            'Remote repo-partial-name current=238de4 staging=499273 desired=4ca939\n'
            'repo-partial-name:\n'
            'repo-partial-name:\n'
            "NEEDS_MORE_COPIES (1) ['repo-backup-name'] wat/test.me.2\n"
            "NEEDS_MORE_COPIES (1) ['repo-backup-name'] wat/test.me.z\n"
            'After:\n'
            'Remote repo-backup-name current=None staging=None desired=7672cb\n'
            'Remote repo-full-name current=b2effc staging=b2effc desired=7672cb\n'
            'Remote repo-incoming-name current=None staging=None desired=None\n'
        ...
        self.assertEqual((
            ...
            'Remote repo-partial-name current=238de4 staging=499273 desired=4ca939\n'
            'repo-backup-name:\n'
            'REMOTE_COPY [repo-partial-name] test.me.1\n'
            'REMOTE_COPY [repo-full-name] test.me.4\n'
            'REMOTE_COPY [repo-partial-name] test.me.5\n'
            'REMOTE_COPY [repo-partial-name] wat/test.me.2\n'
            'REMOTE_COPY [repo-partial-name] wat/test.me.z\n'
            'repo-backup-name:\n'
            'After:\n'
            'Remote repo-backup-name current=7672cb staging=None desired=7672cb\n'
            'Remote repo-full-name current=b2effc staging=b2effc desired=7672cb\n'
            'Remote repo-incoming-name current=None staging=None desired=None\n'
            'Remote repo-partial-name current=238de4 staging=499273 desired=4ca939\n'
            'DONE'), res)
        ...
        self.assertEqual((
            ...
            'Remote repo-full-name current=b2effc staging=b2effc desired=7672cb\n'
            'Remote repo-incoming-name current=None staging=None desired=None\n'
            'Remote repo-partial-name current=4ca939 staging=499273 desired=4ca939\n'
            'DONE'), res)

        self.assertEqual([
            'repo-partial/test.me.1',
            'repo-partial/test.me.5'], dump_file_list(tmpdir=self.tmpdir.name, path="repo-partial"))

        res = await partial_cave_cmd.refresh()
        self.assertEqual((
            'DELETED_NO_COPY wat/test.me.2\n'
            'old: 499273be7613adfbfa809a42a7d2b6e9be0245e6\n'
            'current: 4ca9394ae5678164976a1691dadc8c278a3df2c6\n'
            'Refresh done!'), res)

        res = partial_cave_cmd.status_index(show_files=True, show_dates=False)
        self.assertEqual((
            'test.me.1: present @ -1\n'
            'test.me.5: present @ -1\n'
            '--- SUMMARY ---\n'
            'Result for local [4ca9394ae5678164976a1691dadc8c278a3df2c6]:\n'
            'Max size: 3.5TB\n'
            f'UUID: {partial_cave_cmd.current_uuid()}\n'
            '  # files = 2 of size 14\n')
            , res)
    ...
    async def test_add_fetch_new_repo_after_content_is_in(self):
        ...
        self.assertEqual((
            'Before push:\n'
            'Remote repo-backup-name current=None staging=None desired=d934e1\n'
            'Remote repo-full-name current=1ad9e0 staging=1ad9e0 desired=d934e1\n'
            'Remote repo-incoming-name current=None staging=None desired=None\n'
            'Remote repo-new-contents-name current=b66f52 staging=b66f52 desired=adcdce\n'
            'Remote repo-partial-name current=f9bfc2 staging=f9bfc2 desired=f9bfc2\n'
            'repo-new-contents-name:\n'
            'REMOTE_COPY [repo-partial-name] test.me.2\n'
            ...
            'Remote repo-full-name current=1ad9e0 staging=1ad9e0 desired=d934e1\n'
            'Remote repo-incoming-name current=None staging=None desired=None\n'
            'Remote repo-new-contents-name current=adcdce staging=b66f52 desired=adcdce\n'
            'Remote repo-partial-name current=f9bfc2 staging=f9bfc2 desired=f9bfc2\n'
            'DONE'), res)
        ...
        self.assertEqual((
            ...
            'Remote repo-partial-name current=f9bfc2 staging=f9bfc2 desired=f9bfc2\n'
            'repo-full-name:\n'
            'REMOTE_COPY [repo-new-contents-name] wat/one-new.file\n'
            'repo-full-name:\n'
            'After:\n'
            'Remote repo-backup-name current=None staging=None desired=d934e1\n'
            'Remote repo-full-name current=d934e1 staging=1ad9e0 desired=d934e1\n'
            'Remote repo-incoming-name current=None staging=None desired=None\n'
            'Remote repo-new-contents-name current=adcdce staging=b66f52 desired=adcdce\n'
            'Remote repo-partial-name current=f9bfc2 staging=f9bfc2 desired=f9bfc2\n'
            'DONE'), res)
    ...
    async def test_moving_of_files_in_hoard(self):
        ...
        self.assertEqual((
            'Before push:\n'
            'Remote repo-backup-name current=None staging=None desired=1ad9e0\n'
            'Remote repo-copy-name current=None staging=None desired=1ad9e0\n'
            'Remote repo-full-name current=1ad9e0 staging=1ad9e0 desired=1ad9e0\n'
            'Remote repo-incoming-name current=None staging=None desired=None\n'
            'Remote repo-partial-name current=f9bfc2 staging=f9bfc2 desired=f9bfc2\n'
            'repo-copy-name:\n'
        ...
        self.assertEqual((
            ...
            'Remote repo-copy-name current=1ad9e0 staging=None desired=01152a\n'
        ...
        self.assertEqual((
            ...
            'TO_RETAIN (needed in 2 [repo-backup-name, repo-copy-name]) /test.me.4\n'
            'TO_GET (from 1) /test.me.added\n'
            'TO_RETAIN (needed in 2 [repo-backup-name, repo-copy-name]) /wat/test.me.2\n'
            ' repo-full-name has 5 files\n'
            ' repo-partial-name has 2 files\n'
            'DONE'), res)
        ...
        self.assertEqual((
            'repo-backup-name:\n'
            'TO_GET (from 3) /lets_get_it_started/test.me.2-butnew\n'
            'TO_GET (from 3) /lets_get_it_started/test.me.2-butsecond\n'
    ...
    async def test_moving_of_files_in_hoard_with_backups(self):
        ...
        self.assertEqual((
            ...
            'TO_GET (from 2) /lets_get_it_started/test.me.4-renamed\n'
            'TO_GET (from 2) /test.me.1\n'
            'TO_RETAIN (needed in 1 [repo-backup-name]) /test.me.4\n'
            'TO_GET (from 2) /test.me.added\n'
            'TO_RETAIN (needed in 1 [repo-backup-name]) /wat/test.me.2\n'
        ...
        self.assertEqual((
            ...
            'TO_CLEANUP (is in 3) /wat/test.me.2\n'
        ...
        self.assertEqual((
            'Before push:\n'
            'Remote repo-backup-name current=01152a staging=3a0889 desired=1d6997\n'
            'Remote repo-copy-name current=01152a staging=01152a desired=1d6997\n'
            'Remote repo-full-name current=01152a staging=01152a desired=1d6997\n'
            'Remote repo-incoming-name current=None staging=None desired=None\n'
            'Remote repo-partial-name current=563f87 staging=563f87 desired=7d1569\n'
            'repo-partial-name:\n'
            'repo-partial-name:\n'
            'd wat/test.me.2\n'
            'After:\n'
            ...
            'Remote repo-full-name current=01152a staging=01152a desired=1d6997\n'
            'Remote repo-incoming-name current=None staging=None desired=None\n'
            'Remote repo-partial-name current=7d1569 staging=563f87 desired=7d1569\n'
            'DONE'), res)
    ...
    async def test_moving_of_files_before_first_refresh(self):
        ...
        self.assertEqual((
            'Before push:\n'
            'Remote repo-backup-name current=3a0889 staging=3a0889 desired=e48d88\n'
            'Remote repo-copy-name current=None staging=None desired=e48d88\n'
            ...
            'repo-backup-name:\n'
            'REMOTE_COPY [repo-full-name] lets_get_it_started/test.me.4-renamed\n'
            'REMOTE_COPY [repo-full-name] test.me.1\n'
            'REMOTE_COPY [repo-full-name] test.me.added\n'
            'REMOTE_COPY [repo-partial-name] wat/test.me.2\n'
            'repo-backup-name:\n'
    ...
    async def test_restoring_modified_state_from_hoard(self):
        ...
        self.assertEqual([
            'repo-full-name:',
            'TO_GET (from 1) /test.me.4',
            'TO_RETAIN (needed in 2 [repo-full-name, repo-backup-name]) /test.me.6-moved',
            'TO_GET (from 1) /wat/test.me.2',
            'TO_GET (from 1) /wat/test.me.6',
            ' repo-incoming-name has 2 files',
        ...
        self.assertEqual((
            'Before push:\n'
            'Remote repo-backup-name current=3a0889 staging=3a0889 desired=8da760\n'
            'Remote repo-full-name current=20b513 staging=20b513 desired=8da760\n'
            'Remote repo-incoming-name current=3d1726 staging=3d1726 desired=None\n'
            'Remote repo-partial-name current=f9bfc2 staging=f9bfc2 desired=f9bfc2\n'
            'repo-backup-name:\n'
            'REMOTE_COPY [repo-incoming-name] test.me.4\n'
            'REMOTE_COPY [repo-full-name] test.me.5\n'
            'REMOTE_COPY [repo-partial-name] wat/test.me.2\n'
            'REMOTE_COPY [repo-full-name] wat/test.me.6\n'
            'repo-backup-name:\n'
            'After:\n'
